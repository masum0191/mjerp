...
class SaleController extends Controller
{
    ...
    // api sale store api 
    public function sale_store_api(Request $request)
    {
        $data = $request->all();
        //return $data;
        /*DB::beginTransaction();
        try {*/
            ...
            foreach ($product_id as $i => $id) {
                ...
                $product_sale['sale_unit_id'] = $data['sale_unit_id'];
        ...
        $getinvoice='https://erp.mentarijannah.com/sales/gen_invoices/'.$lims_sale_data->id;
        //sms send end
    ...
    {   $general_setting = GeneralSetting::latest()->first();
        ...
        if($lims_pos_setting_data->invoice_option == 'A4') {
            return view('backend.sale.a4_invoice', compact('lims_sale_data','general_setting', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'paid_by_info', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'totalDue'));
        }
        ...
        elseif($lims_pos_setting_data->invoice_option == 'thermal' && $lims_pos_setting_data->thermal_invoice_size == '58'){
            return view('backend.sale.invoice58', compact('lims_sale_data','general_setting', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'totalDue'));
        }
        else{
            return view('backend.sale.invoice', compact('lims_sale_data', 'general_setting','currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'totalDue'));
        }
    }
    ...
    public function addPayment(Request $request)
    {
        ...
        $lims_payment_data = new Payment();
        $lims_payment_data->user_id = Auth::id();
        $lims_payment_data->sale_create_user_id = $lims_sale_data->user_id;
        $lims_payment_data->sale_id = $lims_sale_data->id;
        if($cash_register_data)
            $lims_payment_data->cash_register_id = $cash_register_data->id;
    ...
    public function addPaymentApi(Request $request)
    {
        $data = $request->all();
        if(!$data['amount'])
            $data['amount'] = 0.00;
        ...
        $lims_payment_data = new Payment();
        $lims_payment_data->user_id =$lims_sale_data->user_id ;
        
        $lims_payment_data->resive_user = Auth::id();
        $lims_payment_data->sale_id = $lims_sale_data->id;
        if($cash_register_data)
            $lims_payment_data->cash_register_id = $cash_register_data->id;
        $lims_payment_data->account_id = $data['account_id'];
        ...
        $lims_payment_data->payment_note = $data['payment_note'];
        $lims_payment_data->payment_receiver = $data['payment_receiver'];
        if(Auth::id()==$lims_sale_data->user_id ){
        $lims_payment_data->status = 1;
        }else{
        $lims_payment_data->status = 2;
        }
        $lims_payment_data->save();
        $lims_sale_data->save();
        ...
        return response()->json([
			'massege'=>'Payment add successfuly',
