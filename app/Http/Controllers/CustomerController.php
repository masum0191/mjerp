...
class CustomerController extends Controller
{
    ...
    public function whatsapp(Request $request)
    {
        //dd($request);
         // Validate the inputs
        //  $request->validate([
        //     'phone' => 'required|regex:/^\+[1-9]\d{1,14}$/', // WhatsApp supports E.164 format
    ...
    public function sendWhatsAppMessageold($phone, $message)
    {
        $client = new Client();
        ...
        try {
            ...
            \Log::error('WhatsApp API Error: Response does not contain messages', ['response' => $responseBody]);
            return false;
    ...
    public function sendWhatsAppMessage()
    {
        ...
        $recipientNumber = 'whatsapp:+8801568282893'; // Replace with the recipient's phone number in WhatsApp format (e.g., "whatsapp:+1234567890")
        $message = "Hello";
    ...
    {
        $recipientNumber = 'whatsapp:+8801635398508'; // Replace with the actual recipient's WhatsApp number
        $message = "Hello from Infobip WhatsApp API in Laravel! 🚀";

        $client = new Client();
        $url = env('INFOBIP_API_URL') . '/whatsapp/1/message/text';

        $payload = [
            'messages' => [
                [
                    'from' => 'whatsapp:+447860099299', // Your shared number (UK number +44)
                    //'from' => '1AD64938DE25A3818114CD5E15DB817E',
                    'to' => $recipientNumber,
                    'text' => ['body' => $message],
                ],
            ],
        ...
        // try {
            ...
            if ($response->getStatusCode() === 200) {
                return response()->json(['message' => 'WhatsApp message sent successfully', 'data' => $responseBody]);
            } else {
                return response()->json(['error' => 'Failed to send message', 'data' => $responseBody], $response->getStatusCode());
            }
    ...
    { 
        // <button type="button" data-id="'.$customer->id.'" class="sendMassege btn btn-link" data-toggle="modal" data-target="#sendMassege" ><i class="dripicons-plus"></i>Send Massege</button>
        $q = Customer::where('is_active', true);
        $totalData = $q->count();
        $totalFiltered = $totalData;
        $massege="Please Payment Your duu bill";
        ...
        if(!empty($customers))
        {
            foreach ($customers as $key=>$customer)
            {
                ...
                $nestedData['options'] .=
                    '<li>
                          <button type="button" class="btn btn-link"> <i class="dripicons-plus"></i> <a href="https://wa.me/+'.$customer->phone_number.'?text='.$massege.'" target="_balnk">Whatsapp</a></button>
    ...
    // customer create api
    public function custom_store_api(Request $request)
    {
    ...
    public function edit_api($id){
        ...
        $lims_customer_group_all = CustomerGroup::where('is_active',true)->get();
        return response()->json([
            'customer'=>$lims_customer_data,
            'customer_group'=>$lims_customer_group_all,
        ]);
    }
    ...
    public function customer_api_update(Request $request, $id)
    {
        $this->validate($request, [
            'phone_number' => [
                'max:255',
