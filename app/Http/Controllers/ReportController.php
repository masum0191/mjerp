...
class ReportController extends Controller
{
    ...
    {
        // Get start and end dates from the request or default to the current month
        $startDate = $request->input('start_date', Carbon::now()->startOfMonth()->format('Y-m-d'));
        $endDate = $request->input('end_date', Carbon::now()->endOfMonth()->format('Y-m-d'));
    
        // Validate dates
        if (!Carbon::canBeCreatedFromFormat($startDate, 'Y-m-d') || !Carbon::canBeCreatedFromFormat($endDate, 'Y-m-d')) {
            return redirect()->back()->withErrors('Invalid date format. Use Y-m-d.');
        }
    
        try {
            // Fetch the sales data by date and biller
            $sales = Sale::getSalesByDateAndBiller($startDate, $endDate);
            ...
            foreach ($sales as $sale) {
                ...
                $report[$date]['billers'][$sale->biller_id] = [
        ...
        return view('backend.report.multiple_dates', compact('report', 'startDate', 'endDate'));
    }
